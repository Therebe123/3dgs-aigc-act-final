...
1. **统一高斯版本（推荐作为格式统一交付）**：A、背景为组员 A 原生 3DGS PLY；B、C 从 OBJ Mesh 表面采样并转换为 3DGS-compatible Gaussian PLY；最终检查后的压缩文件为 `outputs/renders/fused_scene_gaussian_clean_candidate_guitar_floor_v3.ply.gz`。
...
| 物体 B | `assets/object_B/object_B.obj` | OBJ Mesh | 采样后的 Gaussian PLY | 最终融合中使用 94000 个 3DGS-compatible 高斯点 |
| 物体 C | `assets/object_C/object_C.obj` | OBJ Mesh | 采样后的 Gaussian PLY | 最终融合中使用 90000 个 3DGS-compatible 高斯点 |

统一后的高斯场景：`outputs/renders/fused_scene_gaussian_clean_candidate_guitar_floor_v3.ply.gz`，解压后总计 625058 个高斯点。
